...
int ucs2_to_utf8 (char * ucs2, int ucs2_len, char * utf8) {
	...
	for (size_t ucs2_index = 0; ucs2_index < ucs2_len; ucs2_index += 2) {
			uint16_t ucs2_char = (unsigned char)ucs2[ucs2_index];
			...
			} else if (ucs2_char >= 0xD800 && ucs2_char <= 0xDBFF && ucs2_index < ucs2_len - 2) {
				// High surrogate of a surrogate pair
				uint16_t high_surrogate = ucs2_char;
				uint16_t low_surrogate = (unsigned char)ucs2[ucs2_index + 2];
				low_surrogate = (low_surrogate << 8) | (unsigned char)ucs2[ucs2_index + 3];

				if (low_surrogate >= 0xDC00 && low_surrogate <= 0xDFFF) {
					// Valid low surrogate
					uint32_t codepoint = 0x10000 + ((high_surrogate & 0x3FF) << 10) + (low_surrogate & 0x3FF);
			...
			} else if (ucs2_char >= 0xDC00 && ucs2_char <= 0xDFFF && ucs2_index > 0 && ucs2_index < ucs2_len - 1) {
				// Low surrogate of a surrogate pair
				uint16_t low_surrogate = ucs2_char;
				uint16_t high_surrogate = (unsigned char)ucs2[ucs2_index - 2];
				high_surrogate = (high_surrogate << 8) | (unsigned char)ucs2[ucs2_index - 1];
...
int utf8_to_ucs2 (char * utf8, int utf8_len, char * ucs2, int buffer_len) {
	...
	while (utf8_index < utf8_len) {
			unsigned char utf8_char = (unsigned char)utf8[utf8_index++];
			...
			if ((utf8_char & 0x80) == 0) {
				...
				// Two-byte UTF-8 character
				uint16_t codepoint = ((utf8_char & 0x1F) << 6) | ((unsigned char)utf8[utf8_index++] & 0x3F);
				tmp_buff[ucs2_index++] = (uint8_t)(codepoint >> 8);
				tmp_buff[ucs2_index++] = (uint8_t)(codepoint & 0xFF);
			} else if ((utf8_char & 0xF0) == 0xE0) {
				// Three-byte UTF-8 character
				uint16_t codepoint =
						((utf8_char & 0x0F) << 12) | (((unsigned char)utf8[utf8_index++] & 0x3F) << 6) | ((unsigned char)utf8[utf8_index++] & 0x3F);
				tmp_buff[ucs2_index++] = (uint8_t)(codepoint >> 8);
				tmp_buff[ucs2_index++] = (uint8_t)(codepoint & 0xFF);
			} else if ((utf8_char & 0xF8) == 0xF0) {
				// Four-byte UTF-8 character
				uint32_t codepoint =
						((utf8_char & 0x07) << 18) | (((unsigned char)utf8[utf8_index++] & 0x3F) << 12) |
						(((unsigned char)utf8[utf8_index++] & 0x3F) << 6) | ((unsigned char)utf8[utf8_index++] & 0x3F);
			...
			if (rp_data->pdu_len > 0) {
				...
				if (rp_data->pdu.msg_type == SUBMIT || rp_data->pdu.msg_type == DELIVER) {
					...
					if (len > 0) {
						...
						if(len <= 0) {
							LM_ERR("Length of TP-User-Data payload is less than or equal to zero!\n");
							return -1;
						}

						// Check for maximum TP-User-Data payload length since SMS concatenation is not supported
						if((rp_data->pdu.coding == 0x00 && len > 160) || (rp_data->pdu.coding != 0x00 && len > 70)) {
							LM_ERR("Length of TP-User-Data payload exceeds maximum length!\n");
							return -1;
						}
						...
						if (rp_data->pdu.coding == 0x00) {
							...
							j = ucs2_to_utf8(&body.s[p], len, &rp_data->pdu.payload.sm.s[0]);
...
int pv_sms_body(struct sip_msg *msg, pv_param_t *param, pv_value_t *res) {
	...
	if (rp_send_data->pdu.coding == 0x00) {
		...
		// Coding: ucs2
		int ucs2len = utf8_to_ucs2(&rp_send_data->pdu.payload.sm.s[0], rp_send_data->pdu.payload.sm.len, &sms_body.s[sms_body.len], buffer_size - sms_body.len);
		if (ucs2len < 0) {
