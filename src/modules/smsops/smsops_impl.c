...
struct ie_concat_sm_8bit_ref
{
	...
	unsigned char
			max_num_sm; //Maximum number of short messages in the concatenated short message.
...
{
	if(rpdata) {
		if(rpdata->originator.s)
			pkg_free(rpdata->originator.s);
		if(rpdata->destination.s)
			pkg_free(rpdata->destination.s);
		if(rpdata->pdu.originating_address.s)
			pkg_free(rpdata->pdu.originating_address.s);
		if(rpdata->pdu.destination.s)
			pkg_free(rpdata->pdu.destination.s);
		while(rpdata->pdu.payload.header) {
			tp_udh_inf_element_t *next = rpdata->pdu.payload.header->next;
			if(rpdata->pdu.payload.header->identifier
					!= TP_UDH_IE_CONCAT_SM_8BIT_REF) {
				if(rpdata->pdu.payload.header->data.s)
					pkg_free(rpdata->pdu.payload.header->data.s);
			}
			pkg_free(rpdata->pdu.payload.header);
			rpdata->pdu.payload.header = next;
		}
		if(rpdata->pdu.payload.sm.s)
			pkg_free(rpdata->pdu.payload.sm.s);
	}
}
...
static unsigned char gsm7bit_codes[128] = {0x40, 0xA3, 0x24, 0xA5, 0x65, 0x65,
		0xF9, 0xEC, 0x6F, 0x43, 0x0A, 0x4F, 0x6F, 0x0D, 0x41, 0x61, 0xFF, 0x5F,
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x1B, 0xC6, 0xE6,
		0xDF, 0x45, 0x20, 0x21, 0x22, 0x23, 0xA4, 0x25, 0x26, 0x27, 0x28, 0X29,
		0x2A, 0x2B, 0x2C, 0xAD, 0x2E, 0x2F, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35,
		0x36, 0x37, 0x38, 0x39, 0x3A, 0x3B, 0x3C, 0x3D, 0x3E, 0x3F, 0xA1, 0x41,
		0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4A, 0x4B, 0x4C, 0x4D,
		0x4E, 0x4F, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58, 0x59,
		0x5A, 0x41, 0x4F, 0x4E, 0x55, 0xA7, 0xBF, 0x61, 0x62, 0x63, 0x64, 0x65,
		0x66, 0x67, 0x68, 0x69, 0x6A, 0x6B, 0x6C, 0x6D, 0x6E, 0x6F, 0x70, 0x71,
		0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7A, 0x61, 0x6F, 0x6E,
		0x75, 0x61};

static unsigned char gsm7bit_ext_codes[128] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x0C, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
		0xFF, 0xFF, 0xFF, 0x5E, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x7B,
		0x7D, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x5C, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x5B, 0x7E, 0x5D, 0xFF, 0xFF,
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x7C, 0xFF, 0xFF, 0xFF, 0xFF,
...
static int ascii_to_gsm(str sms, char *output_buffer, int buffer_size,
		int *output_text_size, unsigned char paddingBits)
{
	...
	for(; i < sms.len; ++i) {
		if(ascii2gsm7bit_codes[(unsigned char)sms.s[i]] == GSM7BIT_ESCAPE) {
			...
			tmp_buff[out_size++] =
					ascii2gsm7bit_ext_codes[(unsigned char)sms.s[i]];
		} else {
	...
	*output_text_size = out_size;
	...
	if(paddingBits) {
		...
		output_buffer[output_buffer_length++] = tmp_buff[0]
												<< (7 - carry_on_bits);
		carry_on_bits++;
	}
...
int gsm_to_ascii(char *buffer, int buffer_length, str sms, const int fill_bits)
{
	...
	if(fill_bits) {
		...
		symbol =
				((buffer[0] >> fill_bits)
						| // remove the fill bits from the first octet
						(buffer[1]
								& cmask << (8
											- fill_bits)) // mask the required number of bits
	...
	for(; i < buffer_length; ++i) {
		if(carry_on_bits > 0) {
			unsigned char cmask = (1 << (carry_on_bits - 1))
								  - 1; //mask for the rightmost carry_on_bits
									   //E.g. carry_on_bits=3 -> _ _ _ _ _ X X X
			symbol =
					((buffer[i] >> carry_on_bits)
							| //shift right to remove carried bits
							(buffer[i + 1] & cmask)
									<< (8 - carry_on_bits) // carry from the next
			...
			carry_on_bits =
					carry_on_bits
					* -1; //make carry_on_bits positive for the bitwise ops
			unsigned char cmask =
					((1 << carry_on_bits) - 1)
			...
			symbol =
					((buffer[i] << carry_on_bits)
							...
							(buffer[i - 1] & cmask)
									>> (8 - carry_on_bits) // get the bits from the previous octet
							// and shift accordingly
							)
					& BITMASK_7BITS; // mask just 7 bits from the first octet
	...
	for(; i < phone.len; ++i) {
		...
		if(i % 2 == 0) {
			output_buffer[output_buffer_length++] =
					BITMASK_HIGH_4BITS | (phone.s[i] - '0');
		} else {
...
static void DecodeTime(char *buffer, struct tm *decoded_time)
{
	...
	decoded_time->tm_year =
			100 + ((buffer[0] & 0x0F) * 10) + ((buffer[0] >> 4) & 0x0F);
	decoded_time->tm_mon =
			((buffer[1] & 0x0F) * 10) + ((buffer[1] >> 4) & 0x0F) - 1;
	decoded_time->tm_mday =
			((buffer[2] & 0x0F) * 10) + ((buffer[2] >> 4) & 0x0F);
	decoded_time->tm_hour =
			((buffer[3] & 0x0F) * 10) + ((buffer[3] >> 4) & 0x0F);
	decoded_time->tm_min =
			((buffer[4] & 0x0F) * 10) + ((buffer[4] >> 4) & 0x0F);
	decoded_time->tm_sec =
			((buffer[5] & 0x0F) * 10) + ((buffer[5] >> 4) & 0x0F);
}

//The function is called GetXXX but it actually creates the IE if it doesn't exist
static struct ie_concat_sm_8bit_ref *GetConcatShortMsg8bitRefIE(
		sms_rp_data_t *rp_data)
{
				...
				if(_smsops_rp_data->pdu.msg_type == SUBMIT) {
					...
					switch(_smsops_rp_data->pdu.flags & BITMASK_TP_VPF) {
						...
						case BITMASK_TP_VPF_ENHANCED: // 3GPP TS 03.40 9.2.3.12.3 TP-VP (Enhanced format)
							p += 7;
							LM_WARN("3GPP TS 03.40 9.2.3.12.3 TP-VP (Enhanced "
					...
					if(len > 0) {
						if((unsigned char)_smsops_rp_data->pdu.flags
							...
							while(udh_read < udh_len) {
								tp_udh_inf_element_t *ie = pkg_malloc(
								...
								// Check for malicious length, which might cause buffer overflow
								if(udh_read + ie->data.len
												+ 2 /* two octets are read so far */
										> udh_len) {
									LM_ERR("IE Length for IE id %d is bigger "
										   "than the remaining User-Data "
										   "element!\n",
											ie->identifier);
									pkg_free(ie);
									return -1;
								...
								if(ie->identifier
									...
									ie->concat_sm_8bit_ref.max_num_sm =
											body.s[p++];
							...
							if(_smsops_rp_data->pdu.coding == 0x00) {
								...
								len -= (1
										+ udh_len); // add 1 octet for the udh length
							}
						}
...
int dumpRPData(sms_rp_data_t *rpdata, int level)
{
	if(rpdata) {
		...
		LOG(level, "  Originator:                 %.*s (%i)\n",
				rpdata->originator.len, rpdata->originator.s,
				rpdata->originator.len);
		LOG(level, "  Destination flags:          %x (%i)\n",
				rpdata->destination_flags, rpdata->destination_flags);
		LOG(level, "  Destination:                %.*s (%i)\n",
				rpdata->destination.len, rpdata->destination.s,
				rpdata->destination.len);
		LOG(level, "T-PDU\n");
		LOG(level, "  Type:                       %x\n", rpdata->pdu.msg_type);
		LOG(level, "  Flags:                      %x (%i)\n", rpdata->pdu.flags,
				rpdata->pdu.flags);
		...
		LOG(level, "  Originating-Address flags:  %x (%i)\n",
				rpdata->pdu.originating_address_flags,
				rpdata->pdu.originating_address_flags);
		LOG(level, "  Originating-Address:        %.*s (%i)\n",
				rpdata->pdu.originating_address.len,
				rpdata->pdu.originating_address.s,
				rpdata->pdu.originating_address.len);
		LOG(level, "  Destination flags:          %x (%i)\n",
				rpdata->pdu.destination_flags, rpdata->pdu.destination_flags);
		LOG(level, "  Destination:                %.*s (%i)\n",
				rpdata->pdu.destination.len, rpdata->pdu.destination.s,
				rpdata->pdu.destination.len);
		...
		LOG(level, "  Payload:                    %.*s (%i)\n",
				rpdata->pdu.payload.sm.len, rpdata->pdu.payload.sm.s,
				rpdata->pdu.payload.sm.len);
	}
	return 1;
}
	...
	switch(param->pvn.u.isname.name.n) {
			...
			while(ie) {
				if(ie->identifier == TP_UDH_IE_CONCAT_SM_8BIT_REF)
					return pv_get_uintval(msg, param, res,
							(unsigned int)ie->concat_sm_8bit_ref.ref);
				ie = ie->next;
			}
			return -1;
		}
		case SMS_UDH_CONCATSM_MAX_NUM_SM: {
			...
			while(ie) {
				if(ie->identifier == TP_UDH_IE_CONCAT_SM_8BIT_REF)
					return pv_get_uintval(msg, param, res,
							(unsigned int)ie->concat_sm_8bit_ref.max_num_sm);
				ie = ie->next;
			}
			return -1;
		}
		case SMS_UDH_CONCATSM_SEQ: {
			...
			while(ie) {
				if(ie->identifier == TP_UDH_IE_CONCAT_SM_8BIT_REF)
					return pv_get_uintval(msg, param, res,
							(unsigned int)ie->concat_sm_8bit_ref.seq);
				ie = ie->next;
			}
			return -1;
		}
	...
	switch(in->len) {
		case 3:
			if(strncmp(in->s, "all", 3) == 0)
				sp->pvp.pvn.u.isname.name.n = SMS_ALL;
			else
				goto error;
			break;
		case 4:
			if(strncmp(in->s, "type", 4) == 0)
				sp->pvp.pvn.u.isname.name.n = SMS_RPDATA_TYPE;
			else
				goto error;
			break;
		case 9:
			if(strncmp(in->s, "reference", 9) == 0)
				sp->pvp.pvn.u.isname.name.n = SMS_RPDATA_REFERENCE;
			else
				goto error;
			break;
		case 10:
			if(strncmp(in->s, "originator", 10) == 0)
				sp->pvp.pvn.u.isname.name.n = SMS_RPDATA_ORIGINATOR;
			else
				goto error;
			break;
		case 11:
			if(strncmp(in->s, "destination", 11) == 0)
				sp->pvp.pvn.u.isname.name.n = SMS_RPDATA_DESTINATION;
			else
				goto error;
			break;
		case 12:
			if(strncmp(in->s, "origen_flags", 12) == 0)
				sp->pvp.pvn.u.isname.name.n = SMS_RPDATA_ORIGINATOR_FLAGS;
			else
				goto error;
			break;
		case 17:
			if(strncmp(in->s, "destination_flags", 17) == 0)
				sp->pvp.pvn.u.isname.name.n = SMS_RPDATA_DESTINATION_FLAGS;
			else
				goto error;
			break;
		default:
...
int pv_parse_tpdu_name(pv_spec_p sp, str *in)
{
	...
	switch(in->len) {
		...
		case 4:
			if(strncmp(in->s, "type", 4) == 0)
				sp->pvp.pvn.u.isname.name.n = SMS_TPDU_TYPE;
			else
				goto error;
			break;
		case 5:
			if(strncmp(in->s, "flags", 5) == 0)
				sp->pvp.pvn.u.isname.name.n = SMS_TPDU_FLAGS;
			else if(strncmp(in->s, "mp_id", 5) == 0)
				sp->pvp.pvn.u.isname.name.n = SMS_UDH_CONCATSM_REF;
			else
				goto error;
			break;
		case 6:
			if(strncmp(in->s, "coding", 6) == 0)
				sp->pvp.pvn.u.isname.name.n = SMS_TPDU_CODING;
			else if(strncmp(in->s, "origen", 6) == 0)
				sp->pvp.pvn.u.isname.name.n = SMS_TPDU_ORIGINATING_ADDRESS;
			else
				goto error;
			break;
		case 7:
			if(strncmp(in->s, "payload", 7) == 0)
				sp->pvp.pvn.u.isname.name.n = SMS_TPDU_PAYLOAD;
			else
				goto error;
			break;
		case 8:
			if(strncmp(in->s, "protocol", 8) == 0)
				sp->pvp.pvn.u.isname.name.n = SMS_TPDU_PROTOCOL;
			else if(strncmp(in->s, "validity", 8) == 0)
				sp->pvp.pvn.u.isname.name.n = SMS_TPDU_VALIDITY;
			else if(strncmp(in->s, "mp_parts", 8) == 0)
				sp->pvp.pvn.u.isname.name.n = SMS_UDH_CONCATSM_MAX_NUM_SM;
			else
				goto error;
			break;
		case 9:
			if(strncmp(in->s, "reference", 9) == 0)
				sp->pvp.pvn.u.isname.name.n = SMS_TPDU_REFERENCE;
			else
				goto error;
			break;
		case 11:
			if(strncmp(in->s, "destination", 11) == 0)
				sp->pvp.pvn.u.isname.name.n = SMS_TPDU_DESTINATION;
			else if(strncmp(in->s, "mp_part_num", 11) == 0)
				sp->pvp.pvn.u.isname.name.n = SMS_UDH_CONCATSM_SEQ;
			else
				goto error;
			break;
		case 12:
		...
		case 17:
			if(strncmp(in->s, "destination_flags", 17) == 0)
				sp->pvp.pvn.u.isname.name.n = SMS_TPDU_DESTINATION_FLAGS;
			else
				goto error;
			break;
		default:
